    ...
    >
      ...
      <span
        className={classNames(
          ...
          'group-data-selected:bg-accent-600 group-data-selected:group-data-disabled:bg-accent-400', // selected state
          'group-data-disabled:cursor-not-allowed group-data-disabled:bg-gray-200 ', // disabled state
        )}
      >
        {/* toggle circle */}
        {/* prettier-ignore */}
        <span aria-hidden="true" className="pointer-events-none relative inline-flex h-5 w-5 translate-x-0 transform items-center justify-center rounded-full bg-white shadow transition duration-150 group-data-selected:translate-x-5">
          {/* cross icon */}
          <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" className="absolute h-3 w-3 text-gray-400 opacity-100 transition duration-150 group-data-disabled:text-gray-300 group-data-selected:opacity-0">
            <path strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" d="M4 8l2-2m0 0l2-2M6 6L4 4m2 2l2 2" />
          </svg>
          {/* check icon */}
          <svg viewBox="0 0 12 12" fill="currentColor" className="absolute h-3 w-3 text-accent-600 opacity-0 transition duration-150 group-data-disabled:text-accent-400 group-data-selected:opacity-100">
      ...
      {/* label text */}
      <span className="ml-3 text-sm text-gray-900">{props.children}</span>
    </label>
  );
}
